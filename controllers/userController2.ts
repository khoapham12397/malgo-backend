...
import { acceptFriendReq, addUserToGroup, checkFriendShip, checkRelTwoUser, createFriendReq, createGroup, createPostGroup, getFriendList, getFriendReqFrom, getFriendReqTo, getGroupList, getGroupMember, getGroupPostList, getShareResource, lookedShare, shareResource } from "../services/userService2";
...
export const getFriendListCtl = async (req: Request, res: Response) =>{
    try{
        ...
        res.status(error.statusCode).json({successed: false, message: error.message});
    }
}
...
export const getGroupMemberCtl = async (req: Request, res: Response) =>{
    try{
        ...
        if(username && username == req.query.username && typeof(groupId)=='string'){
